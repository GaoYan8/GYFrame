package com.gy.gylibrary.headzoomlayout;
 ...
 * 时间：2019.3.20
 * 描述：可放大头部背景图的布局<br/>
 * 支持RecyclerView、ListView、ScrollView、NestScrollView、LinearLayout等<br/>
 * 定义布局时需要声明头部视图的id{@link HeadZoomLayout#headViewId}，
 * 同时需要将头部中的背景图片（ImageView对象）scaleType设置为centerCrop
 * {@link ImageView#setScaleType(ImageView.ScaleType)}。<br/>
 * 可设置最大下拉距离{@link HeadZoomLayout#mTotalDragDistance}<br/>
 * 可设置头部视图最大拉伸比例{@link HeadZoomLayout#maxZoomRatio}<br/>
 * 可设置是否放大头部{@link HeadZoomLayout#setZoomEnable(boolean)}<br/>
 * 可设置放大头部监听{@link HeadZoomLayout#addOnHeadZoomListener(OnHeadZoomListener)}<br/>
